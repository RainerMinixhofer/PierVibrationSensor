...
def rescale_array(array, new_count):
    """
    ...
    """
    x =  [i*(len(array)-1)/(new_count-1) for i in range(1,new_count-1)]
    x0 = [floor(x[i]) for i in range(0,len(x))]
    x1 = [x0[i]+1 for i in range(0,len(x))]
    dx = [x[i]-x0[i] for i in range(0,len(x))]
    return [array[0]] + [round(array[x0[i]]-dx[i]*(array[x0[i]]-array[x1[i]])) for i in range(0,len(x))] + [array[-1]]
...
class Vibrationsensor:
    ...
    def __init__(self, config):
        ...
        self.tbuffer = 10
        self.ax_ring_buffer = RingBuffer(self.fbuffer * self.tbuffer)
        self.ay_ring_buffer = RingBuffer(self.fbuffer * self.tbuffer)
        self.az_ring_buffer = RingBuffer(self.fbuffer * self.tbuffer)
        self.vx_ring_buffer = RingBuffer(self.fbuffer * self.tbuffer)
        self.vy_ring_buffer = RingBuffer(self.fbuffer * self.tbuffer)
        self.udpip = ""
        self.udpport = 10000
        ...
        self.mpu6500_data_ready = asyncio.ThreadSafeFlag()
        self.adata = bytearray(6)
        ...
        self.ads1256_data_ready = asyncio.ThreadSafeFlag()
        self.vdata = bytearray(3)
    ...
    class ValueGenerator(WebSocketClient):
        ...
        def process(self):
            ...
            try:
                ...
                self.accel = list(self.outer_instance.mpu6500.acceleration)
                ...
                if (previous_accel != self.accel) or (previous_speed != self.speed):
        ...
        try:
            while True:
                ...
                print("ADS1256 Ringbuffer: i=", self.vy_ring_buffer.index, " ri=", self.vy_ring_buffer.retrieve_index,
                      " u=", self.vy_ring_buffer.unread, " f=", self.vy_ring_buffer.is_full, " s=", self.vy_ring_buffer.size)
                if self.vx_ring_buffer.samples_per_second() is not None:
    ...
    def irq_mpu6500(self, enable):
        ...
        """
        if enable:
            self.mpu6500_int_pin.irq(trigger=Pin.IRQ_FALLING, handler=self._irq_mpu6500)
        else:
            self.mpu6500_int_pin.irq(handler=None)

    def _irq_mpu6500(self, _):
        """
        ...
        By asserting the data_ready_interrupt flag we ensure that the interrupt has been triggered by data ready, since some
        interrupts from other sources occur as well.
        """
        if self.mpu6500.data_ready_interrupt:
            self.adata = self.mpu6500.acceleration_raw
            self.mpu6500_data_ready.set()

    async def read_mpu6500data(self):
        """
        Asynchronous function to read the data gathered from the MPU6500 during the call of ISR to the ring buffers.
        Data is given in the following format: [ax, ay, az]
        Stream the data to the UDP socket if the ring buffer has sufficient data given by udppacketsize and the UDP stream is enabled
        """
        try:
            while True:
                await self.mpu6500_data_ready.wait() # type: ignore
                self.ax_ring_buffer.add(self.adata[0])
                self.ay_ring_buffer.add(self.adata[1])
                self.az_ring_buffer.add(self.adata[2])
    ...
    def irq_ads1256(self, enable):
        ...
        """
        if enable:
            self.ads1256_int_pin.irq(trigger=Pin.IRQ_FALLING, handler=self._irq_ads1256)
        else:
            self.ads1256_int_pin.irq(handler=None)

    def _irq_ads1256(self, _):
        """
        ...
        self.vdata = self.ads1256.cycle_channel(ADS1256.CH0 if cur_ch == ADS1256.CH7 else ADS1256.CH7)
        self.ads1256_data_ready.set()

    async def read_ads1256data(self):
        """
        Asynchronous Function to write the data gathered from the ADS1256 during the call of ISR to the ring buffer
        Data is given in the following format: [speedx, speedy]
        """
        try:
            while True:
                await self.ads1256_data_ready.wait() # type: ignore
                cur_ch = self.ads1256.current_channel()
                if cur_ch == ADS1256.CH0:
                    self.vx_ring_buffer.add(self.vdata)
                else:
                    self.vy_ring_buffer.add(self.vdata)
                self.ads1256_data_ready.clear()
        except KeyboardInterrupt:
            pass
    ...
    async def main(self):
        """
        Main function to start the webserver and serve the sensor data through it.
        """
        self.irq_mpu6500(True)
        self.irq_ads1256(True)
#        self.start_webserver()
        try:
            await asyncio.gather(
                self.read_mpu6500data(),
                self.read_ads1256data()
#                self.loop_webserver()
            ) # type: ignore
        except asyncio.CancelledError:
            pass
        finally:
            print("Clean up and stop all interrupts")
            self.irq_mpu6500(False)
            self.irq_ads1256(False)
#            self.stop_webserver()
            sys.exit(0)
    ...
    async def stop(self):
        """
        Stop the main function and clean up resources.
        """
        tasks = [t for t in asyncio.all_tasks() if t is not asyncio.current_task()] # type: ignore pylint: disable=c-extension-no-member
        dummy = [task.cancel() for task in tasks]
